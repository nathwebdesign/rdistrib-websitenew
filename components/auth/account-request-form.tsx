...
import { UserPlus, Building2, Mail, Phone, MapPin, FileText, Lock } from 'lucide-react'
...
export default function AccountRequestForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/mongo/create-request', {
        method: 'POST',
        headers: {
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-gradient-to-r from-primary to-primary/80 p-8 text-white">
          ...
          <h2 className="text-3xl font-bold text-center mb-2">Création de compte</h2>
          <p className="text-center text-white/90">
            Demandez l'accès à votre espace client
          </p>
        </div>
